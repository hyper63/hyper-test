...
export default function (url, headers) {
  const setup = () => $fetch(`${url}/data/test`, { method: 'PUT', headers }).chain(toJSON)

  const update = ({ db, doc }) => () => $fetch(`${url}/data/${db}/${doc.id}`, {
    method: 'PUT',
    headers,
    body: JSON.stringify(doc)
  }).chain(toJSON)

  const create = doc => () => $fetch(`${url}/data/test`, {
    method: 'POST',
    headers,
    body: JSON.stringify(doc)
  }).chain(toJSON)

  const remove = id => () => $fetch(`${url}/data/test/${id}`, {
    method: 'DELETE',
    headers
  }).chain(toJSON)
  ...
  */
  ...
  test('PUT /data/:store/:id - update document should be successful', () =>
    setup()
      ...
      .chain(update({ db: 'test', doc: { id: '63', type: 'test', name: 'foo' } }))
